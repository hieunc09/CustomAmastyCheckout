...
define([
    ...
    Lightboxsocial.prototype = {
        ...
        _makeVisible: function makeVisible() {
            ...
            if (!$noConflict('body').hasClass('social-login-active')) {
                $noConflict('body').addClass('social-login-active')
            }
        },
        ...
        _makeInvisible: function makeInvisible() {
            ...
            if ($noConflict('body').hasClass('social-login-active')) {
                $noConflict('body').removeClass('social-login-active')
            }
        },
    ...
    window.socialLogin = new Lightboxsocial('magestore-popup');
    var links = document.links;
    var socialLoginLinkPopup = $('sociallogin-other-a-popup');
    var socialLoginCreateBackTo = $('magestore-create-back');
    var socialLoginWrapper = $('magestore-login-social');
    var socialLoginCreateTabBtn = $('magestore-sociallogin-create-new-customer');
    ...
    for (i = 0; i < links.length; i++) {
        if (links[i].href.search('/customer/account/login/') !== -1
            && links[i].href.search('/customer/account/login/#') === -1
        ) {
            links[i].href = 'javascript:socialLogin.open();';
            Event.observe(links[i], 'click', function () {
                socialLoginCreateBackTo.click();
                socialLoginWrapper.style.display = "block";
                if (socialLoginLinkPopup)
                    socialLoginLinkPopup.style.display = "block";
            });
        }
